...
expert users.
%
\CHANGED{Finally, we present the first standardized benchmark for proofreading in connectomics as an open research platform.}

Guided proofreading is applicable to all existing automatic segmentation methods that
produce a label map.
\CHANGED{While we train models based on traditional CNN architectures, we propose an error correction framework with human interaction that works in principle with any potential classifier.}
