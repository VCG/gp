...
edge. \CHANGED{These classifiers automatically reduce error with little training data,} which is
expensive to collect for connectomics data.
%
Second, we develop a \CHANGED{human-}guided proofreading approach to correcting segmentation
...
expert users.
%
\CHANGED{Fourth, we present the first connectomics proofreading benchmark, with image, label, and human interaction data, and evaluation code.}

Guided proofreading is applicable to all existing automatic segmentation methods that produce a label map.
\CHANGED{While we train models based on traditional CNN architectures, we propose an error correction approach with human interaction that works with many classifiers.}
