...
&
\begin{tabular}{l}
\textbf{Results} \\ cost [m]: 217 \\ ~ \\ ~ \\ Test. acc.: 0.99 \\ Prec./Recall: \HP{add}?/? \\ F1 Score: \HP{add}?
 \end{tabular}
\end{tabular}
\vspace{1mm}
\caption{Training parameters, cost, and results of our guided proofreading classifier versus focused proofreading by Plaza~\cite{focused_proofreading}. Both methods were trained on the same mouse brain dataset using the same hardware (Tesla K40 GPU).\HP{Maybe split this into two tables?}}%While the training of our classifier is more expensive, testing accuracy is superior. }
