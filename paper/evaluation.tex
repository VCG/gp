...
The following serial section electron microscopy datasets are used for the evaluation of our method. 
\\~\\
...
\\~\\
\textbf{CREMI A/B/C.} As part of the MICCAI 2016 challenge on circuit reconstruction from electron microscopy images (CREMI) three datasets (each $1250\times1250\times125$ voxels) were made publicly available including manually labeled ground truth\footnote{The MICCAI CREMI challenge data is available at http://www.cremi.org.}. The volumes are part of adult fruit fly (Drosophila melanogaster) brain. The resolution of all three datasets is $4\times4\times40~\text{nm}^3\text{/voxel}$. 
\\~\\
...
~\\
 ...
 \\
 \begin{tabular}{l}
\textbf{Results} \\ cost [m]: 383 \\ Val. loss: 0.0845 \\ Val. acc.: 0.969 \\ Test. acc.: 0.94 \\ Prec./Recall: 0.94/0.94 \\ F1 Score: 0.94
 \end{tabular}
&
\begin{tabular}{l}
\textbf{Results} \\ cost [m]: 217 \\ ~ \\ ~ \\ Test. acc.: 0.99 \\ Prec./Recall: ?/? \\ F1 Score: ?
 \end{tabular}
\end{tabular} 
\vspace{1mm}
...
\textbf{Interactive proofreading.} According to the comparison of Haehn~\etal~, the best performing proofreading tool for novices is Dojo. We use the publicly available findings of their user study as a baseline comparison.

\textbf{Computer-aided proofreading.} FP
