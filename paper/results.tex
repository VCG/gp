...
Additional plots \CHANGED{and confirmatory data analysis} are available as supplemental material due to limited space. 
...
\CHANGED{We present the first proofreading benchmark for the field of connectomics as an open research platform. We include representative connectomics data with ground truth labels, all results and participant demographics from the interactive experiments by Haehn~\etal~\cite{haehn_dojo_2014}, our complete results from this paper, as well as standardized metrics for comparing different proofreading approaches at (link omitted for review). We hope that this framework facilitates future developments and allows comparisons for conquering the important proofreading problem in connectomics.}
