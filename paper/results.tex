...
\paragraph{L.~Cylinder.} Evaluation was performed on previously unseen sections of the mouse cortex volume from Kasthuri~\etal~\cite{kasthuri2015saturated}. We generated an unbalanced dataset of 81,184 correct and 8,780 split error patches with respect to the ground truth labeling. Then, we ranked each patch by using focused proofreading and guided proofreading, and compare precision/recall (Fig. \ref{fig:pr}). Our method exhibits higher precision and recall.% for correct and erroneous patches.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{gfx/precisionrecall.pdf}
\caption{Precision-recall curves comparing focused proofreading (FP) and our guided proofreading (GP) classifiers. We evaluate on unbalanced tests sets of the AC4 subvolume (darker colors) and the L. Cylinder volume (lighter colors). GP performs better.}
\label{fig:pr}
\end{figure}

\paragraph{AC4 subvolume.} We generated 3,488 correct and 332 error patches (10 merge errors, 322 split errors). Guided proofreading achieves better precision/recall scores (Fig. \ref{fig:pr}).
...
\begin{table}[h]
\caption{Average proofreading speed for users of Dojo, Focused Proofreading (FP) and our Guided Proofreading (GP). Our system achieves significantly higher VI reduction per minute (7.5$\times$) over state-of-the-art FP, while being slightly slower per correction.}%While the training of our classifier is more expensive, testing accuracy is superior. }
\resizebox{\linewidth}{!}{
\begin{tabular}{lrrrr}
\toprule
Approach & \makecell{Time Per\\Correction (s)} & \makecell{VI Reduction\\Per Minute} & \makecell{Improvement} \\
\midrule
