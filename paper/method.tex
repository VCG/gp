...
\CHANGED{We observed that correct splits tend to include label boundaries, and so we promote their inclusion by we dilating the label by 20 pixels. This lets the watershed algorithm `pick up' the boundary more successfully. These boundaries are then individually rated using our split error classifier (Fig.~\ref{fig:merge_error}; pseudo code in supplemental).}
