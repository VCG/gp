...
If the classifier yields a result close to 0, the candidate split is likely to be correct. Then, we use it to fix the merge error.}
...
As preprocessing, we dilate the label by 20 pixels. This is motivated by our observation that the generated splits tend to attach to real membrane boundaries. These boundaries are then individually rated using our split error classifier (Fig.~\ref{fig:merge_error}).  Pseudo code is available in supplemental material.}
