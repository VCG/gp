...
\CHANGED{To find merge errors, we generate candidate splits within a label segment and use our split error classifier to rate them. 
...
If the classifier yields a result close to 0, the candidate split is likely to be correct. We then use it to fix the merge error.}
...
\CHANGED{%For each segmentation label, we generate 50 potential boundaries through the region by randomly placing watershed seed points at opposite sides of the label boundary. 
As preprocessing, we dilate the label by 20 pixel which is motivated by our observation that the generated splits tend to attach to real membrane boundaries. These boundaries are then individually rated using our split error classifier. Fig. \ref{fig:merge_error} illustrates this procedure. Pseudo code is available as supplemental material to promote understanding.}
